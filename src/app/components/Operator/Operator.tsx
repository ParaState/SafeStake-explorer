...
const Operator = () => {
  ...
  const loadOperator = (address: string, periods: string[] = ['1days'], only_performance: boolean = false) => {
    ...
    DvfNetwork.getInstance().fetchOperator(address, periods).then((result: any) => {
  ...
  const loadOperatorValidators = (address: string, paginationPage: number) => {
    ...
    DvfNetwork.getInstance().fetchOperatorValidators(address, page, perPage).then((result: any) => {
      if (result.status === 404) {
        setNotFound(true);
      } else {
