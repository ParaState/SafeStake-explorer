...
export default (props: ValidatorDutiesProps) => {
  ...
  const loadValidatorDuties = (address: string, page: number) => {
    ...
    DvfNetwork.getInstance().fetchValidatorDuties(address, currentPage, perPage).then((result: any) => {
          ...
          <SuccessChip
            ...
            label={successOperators.map((o, oi) => (
              <ChipLink
                ...
                style={{ maxWidth: 100 }}
              >
                <Typography component={'div'} noWrap style={{ fontSize: 14 }}>{o.name}</Typography>
              </ChipLink>
            ))
            }
            onDelete={() => {}}
