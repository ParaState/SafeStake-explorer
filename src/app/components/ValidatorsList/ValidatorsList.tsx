...
const ValidatorsList = () => {
  ...
  const loadValidators = (paginationPage?: number) => {
    ...
    setLoading(true);
    DvfNetwork.getInstance().fetchValidators(page, perPage, true).then((result: any) => {
  ...
  return (
    <Layout>
      <ContentContainer>
        ...
        <Typography component={'div'} variant="h1" style={{ color: '#3F3ACA' }}>Validators</Typography>
            ...
            return [
              <Link href={`${config.routes.VALIDATORS.HOME}/${validator.public_key}`} className={classes.Link}>
                <Box component="div" display={{ xs: 'block', sm: 'block', md: 'none', lg: 'none' }}>
                  {longStringShorten(validator.public_key)}
                </Box>
