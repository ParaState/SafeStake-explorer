...
const OperatorsList = () => {
  ...
  const loadOperators = (paginationPage?: number) => {
    ...
    setLoading(true);
    DvfNetwork.getInstance().fetchOperators({ page, perPage, validatorsCount: 'true', status: 'true' })
      .then((result: any) => {
  ...
  const getOperatorsTableHeaders = () => {
    const headers = [
      ...
      <div>
        Status
        <InfoTooltip
          style={{ ...infoIconStyle, color: '#fffff', marginBottom: -2 }}
  ...
  return (
    <Layout>
      <ContentContainer>
        ...
        <Typography component={'div'} variant="h1" style={{ color: '#4DC9F0' }}>Operators</Typography>
